...
import { Pengumuman, PengumumanPagination, PengumumanParams, PengumumanReq } from "@/models/PengumumanModel";

export const fetchPengumumanAdmin =
  async (params: PengumumanParams): Promise<PengumumanPagination> => {
    const response = await api.get<PengumumanPagination>(
      "/pengumuman/admin", {
        params
      }
    );
...
export const fetchPengumumanUser =
  async (): Promise<PengumumanPagination> => {
    const response = await api.get<PengumumanPagination>(
      "/pengumuman"
    );
    return response.data;
  ...
  export const updatePengumuman = async (id: string, request: PengumumanReq) => {
    const response = await api.put<PengumumanReq>("/pengumuman/admin/" + id, request);
    return response;
  };
