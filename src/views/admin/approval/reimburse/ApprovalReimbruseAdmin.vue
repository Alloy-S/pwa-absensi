    ...
    <BasePage>
        ...
        <div class="bg-white p-4 rounded-lg shadow-md">
            <DataTable :value="reimburseList" lazy paginator :rows="lazyParams.rows" :rowsPerPageOptions="[5, 10, 20, 50]"
                ...
                <Column field="created_date" header="Tanggal Pengajuan" style="width: 25%">
                ...
                <template #empty>
                    <div class="text-center py-5">
                        <p class="text-gray-500">Tidak ada data pengajuan reimburse yang ditemukan.</p>
                    </div>
                </template>
            </DataTable>
        </div>
...
import { fetchReimburseAdmin } from '@/services/reimburseService';
...
const getReimburse = async () => {
    ...
    try {
        ...
        const response = await fetchReimburseAdmin(params);
        reimburseList.value = response.items;
        totalRecords.value = response.total;
    } catch (error) {
        console.error("Gagal memuat data reimburse.");
    } finally {
        loading.value = false;
    }
};
...
const goToDetail = (id: string) => {
    router.push(`/admin/approval/reimburse/${id}`);
};
