  ...
  <BasePage>
      ...
      <div class="space-y-6">
          <div class="flex items-center justify-between">
              <div>
                  <h1 class="text-3xl font-semibold text-slate-800">Detail Approval Koreksi</h1>
              </div>
          ...
          <div v-else-if="koreksiDetail" class="space-y-4">
              <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  <div class="lg:col-span-2 bg-white p-4 rounded-lg shadow-md">
                      ...
                      <p class="text-sm text-slate-600">{{ koreksiDetail.user.jabatan }} - {{ koreksiDetail.user.lokasi }}</p>
                  </div>
              </div>

              
              <div class="bg-white p-4 rounded-lg shadow-md">
                  <h3 class="font-semibold text-slate-800 mb-2">Pengajuan untuk Tanggal:</h3>
                  <p class="font-bold text-lg text-blue-600 mb-4">{{ formatDate(koreksiDetail.absensi_date) }}</p>
                  
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                      
                      <div class="border-r pr-6">
                          <h4 class="font-semibold text-gray-700 mb-3">Jadwal Seharusnya ({{ koreksiDetail.jadwal_kerja.shift }})</h4>
                          <div class="space-y-2 text-sm">
                              <p><span class="font-medium w-24 inline-block">Masuk</span>: {{ jadwalHariIni?.time_in }}</p>
                              <p><span class="font-medium w-24 inline-block">Pulang</span>: {{ jadwalHariIni?.time_out }}</p>
                          </div>
                      </div>
                      ...
                      <div>
                          <h4 class="font-semibold text-green-700 mb-3">Waktu Baru yang Diajukan</h4>
                           <div class="space-y-2 text-sm">
                              <p><span class="font-medium w-24 inline-block">Masuk</span>: <span class="font-bold">{{ formatTime(jamMasuk?.requested_datetime) }}</span></p>
                              <p><span class="font-medium w-24 inline-block">Pulang</span>: <span class="font-bold">{{ formatTime(jamPulang?.requested_datetime) }}</span></p>
                          </div>
                      </div>
                  </div>
              </div>
              ...
              <div v-if="koreksiDetail.status.toLowerCase().includes('menunggu')" class="flex justify-end space-x-3 pt-4">
...
const jamMasuk = computed<DetailKoreksi | undefined>(() => {
  return koreksiDetail.value?.detail_approval.find(d => d.type === 'IN');
});
const jamPulang = computed<DetailKoreksi | undefined>(() => {
...
const confirmAction = (action: 'approve' | 'reject') => {
  ...
  confirm.require({
      message: `Apakah Anda yakin ingin ${isApprove ? 'menyetujui' : 'menolak'} pengajuan koreksi ini?`,
...
const handleAction = async (action: 'approve' | 'reject') => {
  ...
  try {
      ...
      router.push('/admin/approval/koreksi-kehadiran');
  } catch (error) {
