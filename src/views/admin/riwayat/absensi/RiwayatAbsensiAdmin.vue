...
import { Absensi, AbsensiParams } from '@/models/absensiModel';
...
const getHistory = async () => {
    loading.value = true;
    try {
        const params: AbsensiParams = {
            page: lazyParams.value.page,
            size: lazyParams.value.rows,
            search: searchQuery.value,
            "filter-month": selectedMonth.value,
        };
        const response = await fetchHistoryAbsensiAdmin(params);
        absensiList.value = response.items;
...
const goToDetail = (id: string) => {
    router.push(`/admin/riwayat/absensi/${id}`);
};
...
const statusBadgeColor = (status: string) => {
    ...
    if (s.includes('terlambat') || s.includes('cepat')) return 'bg-yellow-100 text-yellow-800';
    if (s.includes('hadir')) return 'bg-green-100 text-green-800';
    if (s.includes('alpha')) return 'bg-red-100 text-red-800';
