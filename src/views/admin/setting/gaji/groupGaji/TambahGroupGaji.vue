    ...
    <BasePage>
        <div class="space-y-3">
            <div class="mb-10 mt-5 flex justify-between items-start">
                <p class="text-3xl font-semibold text-slate-800">Tambah Grup Gaji</p>
            </div>

            <form @submit.prevent="simpan" class="flex flex-col lg:flex-row lg:space-x-5">
                <div class="flex-[7] space-y-5">
                    <div class="p-5 bg-white rounded-md shadow-md space-y-4">
                        <div>
                            ...
                            <input v-model="form.grup_kode" type="text" required maxlength="5"
...
import { addGrupGaji, fetchAllKodePerhitungan } from '@/services/GrupGajiService';
...
const getMasterKomponen = async () => {
    try {
        const [komponenRes, kodeRes] = await Promise.all([
            fetchAllKomponen(),
        ...
        toast.error("Gagal memuat daftar master komponen gaji.");
    }
};
...
const simpan = async () => {
    ...
    try {
        await addGrupGaji(form.value);
        toast.success("Grup gaji berhasil ditambahkan.");
        router.push('/admin/pengaturan/gaji/grup-gaji');
    } catch (error) {
        toast.error("Gagal menyimpan grup gaji.");
    } finally {
...
onMounted(() => {
    getMasterKomponen();
    if (form.value.komponen.length === 0) {
        addKomponen();
    }
});
</script>
