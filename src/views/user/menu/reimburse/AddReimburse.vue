<template>
    <BasePageNoNav>
        ...
        <div class="p-4">
            ...
            <p class="text-sm text-slate-500 mb-6">Lengkapi detail item dan unggah bukti foto untuk mengajukan
                reimburse.</p>
            ...
            <form @submit.prevent="submitReimburse" class="space-y-6">
                ...
                <div>
                    ...
                    <div v-if="items.length === 0"
                        class="text-center text-sm text-gray-500 p-4 border-2 border-dashed rounded-lg">
                        Belum ada item ditambahkan.
                    </div>
                    <div v-else class="space-y-3">
                        <div v-for="(item, index) in items" :key="index"
                            class="bg-white border rounded-lg p-4 relative">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="text-sm text-right border-t pt-2 mt-2">
                                    Subtotal: <span class="font-semibold text-slate-800">{{ formatCurrency(item.jumlah *
                                        item.harga || 0) }}</span>
                                </div>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <div v-if="photoBase64" class="mt-4">
                        ...
                        <img :src="photoBase64" alt="Preview"
                            class="mt-2 w-full max-w-xs mx-auto rounded-md border shadow-sm">
                    </div>
                </div>
...
const handleFileUpload = (event: Event) => {
    ...
    reader.onload = async (e) => {
        ...
        try {
            const compressedBase64 = await compressImage(originalBase64, 500);
            photoBase64.value = compressedBase64;
            toast.remove(toastId);
        } catch (error) {
            console.error("Gagal mengompres gambar:", error);
            toast.update(toastId, { render: "Gagal mengompres gambar.", type: "error", isLoading: false, autoClose: 3000 });
            photoBase64.value = originalBase64;
        } finally {
